...
public class StyledLayer {
    ...
    public void setBasicStyle(BasicStyle style) {
        if (styles != null) {
            List<PStyle> stylesToBeRemoved = new ArrayList<>();
        ...
        for (Category cat : categories) {
            ...
            for (Layerable lyr : layerables) {
                ...
                if (accept(category, layer, cat, lyr)) {
                    Logger.info("accepted : apply styles");
                    applyStyles(lyr);
                }
            }
        }
     ...
     * @param lyr a layerable
     */
    public void applyStyles(Layerable lyr) {
        Logger.info("Apply styles to " + lyr);
        ...
        if (lyr instanceof Layer) {
            ...
            if (styles != null) {
                Logger.info("Nombre de styles enregistrés : " + styles.size());
                for (PStyle pstyle : styles) {
                    try {
                        ...
                        } else if (newStyle instanceof BasicStyle) {
                            layer.removeStyle(layer.getBasicStyle());
                        }
                        Logger.info("  - replace " + newStyle.getClass().getSimpleName());
                        layer.addStyle(newStyle);
                    } catch (Exception e) {
                        Logger.warn(pstyle.toString(), e);
                    }
                }
            }
